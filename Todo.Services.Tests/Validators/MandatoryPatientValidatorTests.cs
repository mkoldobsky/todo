...
{
    [TestFixture]
    public class MandatoryPatientValidatorShould
    {
        [Test]
        public void GetInfoMessageWhenNotFieldsToValidate()
        {
            var service = new Mock<ICircuitService>();
            service.Setup(x => x.GetMandatoryPatientFieldsByFormName("test")).Returns(new List<string>()).Verifiable();
            var values = new Dictionary<string, string>();
            values["key1"] = "value1";
            values["key2"] = "value2";
            var validator = new MandatoryPatientValidator(service.Object);
            var result = validator.Validate("test", values);
            Assert.IsNotNull(result);
            Assert.AreEqual(1, result.Count);
            service.Verify(x => x.GetMandatoryPatientFieldsByFormName("test"), Times.Once);
        ...
        {
            var service = new Mock<ICircuitService>();
            service.Setup(x=>x.GetMandatoryPatientFieldsByFormName("test")).Returns(new List<string>{"key1", "key2"}).Verifiable();
            var values = new Dictionary<string, string>();
            values["key1"] = "value1";
            values["key2"] = "value2";
            var validator = new MandatoryPatientValidator(service.Object);
        ...
        public void NotValidateWhenNotContainsAllMandatoryFields()
        {
            ...
            Assert.AreEqual("El paciente no se puede retirar", result[1].Message);
        }
    }
}
